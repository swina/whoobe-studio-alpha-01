<template>
    <div class="fixed z-top top-0 left-0 h-screen w-10 shadow theme-dark flex flex-col items-start justify-start pt-10 border-r border-purple-900">
        <div v-for="item in menu.items" class="h-10 w-10 mb-2 flex flex-col items-center justify-center hover:text-gray-200 hover:bg-indigo-700 cursor-pointer relative" :title="item.label" @mouseover="label=item.label" :key="item.label" @click="addTab(item)">
            ...
            <transition name="fade">
                <div v-if="item.items && label===item.label" class="absolute top-0 left-0 ml-10  z-highest w-48 bg-gray-800 flex flex-col" @mouseleave="label=null">
                    <div class="bg-indigo-700 h-10 p-1 items-center flex">{{ item.label }}</div>
                    <div v-for="sub in item.items" :key="sub.label" @click="addTab(sub)">
                        <div class="border-l-4 border-transparent hover:border-indigo-700 hover:bg-gray-700 p-1 hover:text-gray-200 capitalize text-base">{{ sub.label }}</div>
                    </div>
                </div>
            </transition>
            <transition name="fade">
            <div v-if="item.dataset && label===item.label" class="absolute top-0 left-0 ml-10  z-highest w-48 bg-gray-800 flex flex-col" @mouseleave="label=null">
                <div class="bg-indigo-700 h-10 p-1 items-center flex border-l">{{ item.label }}</div>
        ...
        <template v-for="plugin in datastore.dataset.plugins">
            <div v-if="plugin.general.help" class="h-10 w-10 mb-2 flex flex-col items-center justify-center hover:text-gray-200 hover:bg-indigo-700 cursor-pointer relative" :title="plugin.general.name" :key="plugin.general.name" @mouseover="label=plugin.general.name" @click="addPluginTab(plugin)">
                <icon :name="plugin.general.version" :label="plugin.general.name"/>
            </div>
        </template>
    </div>
</template>
...
export default {
    ...
    methods:{
        addTab ( item  ){
            ...
            if ( this.desktop.tabs.length ){
                this.desktop.tabs.forEach ( (tab,index) => {
                    console.log ( tab.name , item.label , index )
                    if ( tab.name === item.label  ){
